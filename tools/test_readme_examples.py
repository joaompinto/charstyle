...
def split_into_examples(code_block: str) -> List[Tuple[str, str]]:
    ...
    for i in range(len(header_indices)):
        ...
        if example_code.strip() and not all(line.strip().startswith('#') for line in example_code.strip().split('\n')):
...
def test_example(example: CodeExample, extra_imports: List[str] = None) -> Tuple[bool, str]:
    ...
    # Add standard imports and path setup
    setup_code = [
        "import sys",
        "import os",
        "import io",
    ...
    # Add common charstyle imports that might be needed for examples
    charstyle_imports = [
        "from charstyle import (RED, GREEN, BLUE, YELLOW, BOLD, ITALIC, UNDERLINE,",
        "    BG_RED, BG_GREEN, BG_BLUE, BG_BLACK, BG_YELLOW, BRIGHT_RED, BRIGHT_GREEN,",
        "    colored, Style, Icon, style_split, style_complex, style_pattern_match, style_format,",
        "    red, green, blue, yellow, bold, italic, underline)",
    ]
...
def main() -> int:
    ...
    for i, (code_block, line_number, section_header) in enumerate(code_blocks):
        ...
        print(f"Testing block {i+1} (starting at line {line_number}) [{section_header}]... ", end="")
